...
void CDebugBonePanel::DrawSkeletonOverlay_ImGui(CGameObject* target, const ImVec2& vpPos, const ImVec2& vpSize)
{
	...
	vector<Matrix> boneModel(boneCount);
	

	//// 오너 월드 행렬
	//Matrix ownerWorld(*(const DirectX::XMFLOAT4X4*)pTr->Get_WorldMatrix_Ptr());
	//
	//// 선택 본 조상 체인(강조용)
	//std::vector<uint8_t> isAncestor(boneCount, 0);
	...
	//
	//std::vector<ImVec2> screen(boneCount);
	//std::vector<bool>   ok(boneCount, false);
	//
	//// 본 월드 위치 투영
	...
	//	ok[i] = WorldToScreen_SM(pos, view, proj, vpPos, vpSize, screen[i]);
	//}
	//
	//// 라인(부모-자식)
	//for (int i = 0; i < boneCount; ++i)
