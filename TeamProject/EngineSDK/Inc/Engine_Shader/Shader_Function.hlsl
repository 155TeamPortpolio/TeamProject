...
float GeometrySchlickGGX(float NdotV, float roughness)
{
    ...
    float k = (r * r) / 8.f; //환경광이 있다면 IBL로 바뀜
...
float3 view, float3 light, float3 lightcolor, float lightIntensity)
{
    ...
    float3 normalizeview = normalize(view);
    float3 normalizelight = normalize(light);
    float3 normalizehalfdir = normalize(normalizeview + normalizelight);
    
    float NdotV = saturate(dot(normalizenormal, normalizeview));
    float NdotL = saturate(dot(normalizenormal, normalizelight));
    float HdotV = saturate(dot(normalizehalfdir, normalizeview));
    
    float3 F0 = CalculateF0(albedo, metalic);
    float D = DistributionGGX(normalizenormal, normalizehalfdir, roughness);
    float G = GeometrySmith(normalizenormal, normalizeview, normalizelight, roughness);
    float3 F = FresnelSchlick(HdotV, F0);
    
    float3 numerator = D * G * F;
    float denominator = 4.f * NdotV * NdotL + 0.0001;
    ...
    float3 Luminanceout = BRDF * lightcolor * lightIntensity * NdotL;
