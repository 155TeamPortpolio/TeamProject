#include "pch.h"
#include "UI_LotteryResultBanner.h"

#include "GameInstance.h"
#include "ObjectContainer.h" 
#include "TextSlot.h"

#include "UI_IconButton.h"

HRESULT CUI_LotteryResultBanner::Initialize_Prototype()
{
    __super::Initialize_Prototype();

    Add_Component<CObjectContainer>();

    return S_OK;
}

HRESULT CUI_LotteryResultBanner::Initialize(INIT_DESC* pArg)
{
    __super::Initialize();

    Load(Helper::LoadJson<nlohmann::ordered_json>(ResourceManager()->Get_ResourcePath("lottery_result_banner.json")));
    Cache();

    Create_ConfirmButton();

    Set_Alive(false);

    return S_OK;
}

void CUI_LotteryResultBanner::Awake()
{
}

void CUI_LotteryResultBanner::Update(_float dt)
{
    __super::Update(dt);

    Get_Component<CObjectContainer>()->UpdateChild(dt);

    if (m_eState == STATE::INVISIBLE && Is_AnimFinished())
        Set_Alive(false);
}

void CUI_LotteryResultBanner::UI_Active(void* pArg)
{
    Change_State(STATE::VISIBLE);

    _int iDenny = {};
    if (pArg)
    {
        RESULT_DESC* pDesc = static_cast<RESULT_DESC*>(pArg);
        if (pDesc)
            iDenny = pDesc->iDenny;
    } 

    Set_Text(TEXTSLOT::DENY, Helper::ConvertToWideString(to_string(iDenny)));
}

void CUI_LotteryResultBanner::UI_DeActive(void* pArg)
{
    Change_State(STATE::INVISIBLE);
}

void CUI_LotteryResultBanner::Cache()
{
    auto pContainer = Get_Component<CObjectContainer>();

    for (_int i = 0; i < ENUM(TEXTSLOT::END); ++i)
    {
        auto pObj = pContainer->Find_Descendant(INSTANCENAMES[i]);
        if (!pObj)
            continue;

        m_pTextSlots[i] = pObj->Get_Component<CTextSlot>();
    }
}

void CUI_LotteryResultBanner::Create_ConfirmButton()
{
    CUI_IconButton::BUTTON_DESC* pDesc = new CUI_IconButton::BUTTON_DESC;
    pDesc->onClick = [this]() { OnClick_Confirm(); };
    pDesc->strLabel = L"È®ÀÎ";
    pDesc->strTextureKey = "IconOK.png";
    auto pObj = Builder::Create_UIObject({ G_GlobalLevelKey, "Proto_GameObject_IconButton" })
        .Add_UIDesc(pDesc)
        .Build("buttonOK");

    if (!pObj)
        return;

    Get_Component<CObjectContainer>()->Add_Child(pObj);
}

void CUI_LotteryResultBanner::Change_State(STATE eState)
{
    if (m_eState == eState)
        return;

    m_eState = eState;
    switch (eState)
    {
    case STATE::INVISIBLE:
        Set_Animation(1);
        break;
    case STATE::VISIBLE:
        Set_Alive(true);
        Set_Animation(0);
        break;
    }
}

void CUI_LotteryResultBanner::OnClick_Confirm()
{
    UI_DeActive();
    if (m_OnClick)
        m_OnClick();
}

void CUI_LotteryResultBanner::Set_Text(TEXTSLOT textSlot, const _wstring& strText)
{
    auto pTextSlot = m_pTextSlots[ENUM(textSlot)];
    if (!pTextSlot)
        return;

    pTextSlot->Set_Text(strText);
}

CGameObject* CUI_LotteryResultBanner::Create()
{
    CUI_LotteryResultBanner* pInstance = new CUI_LotteryResultBanner();
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Create : CUI_LotteryResultBanner");
        Safe_Release(pInstance);
    }
    return pInstance;
}

CGameObject* CUI_LotteryResultBanner::Clone(INIT_DESC* pArg)
{
    CUI_LotteryResultBanner* pInstance = new CUI_LotteryResultBanner(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Clone : CUI_LotteryResultBanner");
        Safe_Release(pInstance);
    }
    return pInstance;
}