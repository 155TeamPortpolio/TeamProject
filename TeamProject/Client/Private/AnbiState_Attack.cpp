...
void CAnbiState_Attack::Enter(CAnbi* pOwner)
{
    if (!m_pSubStateMachine)
    {
        m_pSubStateMachine = CStateMachine<CAnbi>::Create();
        m_pSubStateMachine->Register_State("NormalAttack", CAnbiState_NormalAttack::Create());
...
void CAnbiState_Attack::Update(CAnbi* pOwner, _float dt)
{
    ...
    if (m_pSubStateMachine)
    {
        ...
        {
            CAnbiState_NormalAttack* pNormalAttack =
                static_cast<CAnbiState_NormalAttack*>(m_pSubStateMachine->Get_State("NormalAttack"));
...
void CAnbiState_Attack::Exit(CAnbi* pOwner)
{
}

_bool CAnbiState_Attack::Handle_Transition(CAnbi* pOwner, const string& strState)
{
    if (strState != "Attack")
    {
        if (!m_pSubStateMachine)
...
void CAnbiState_Attack::Move_Motion(CAnbi* pOwner, _float dt)
{
