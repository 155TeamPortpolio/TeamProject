...
CStrikeJaeger::CStrikeJaeger(const CStrikeJaeger& rhg)
	: CEnemyNormal(rhg)
{
}

HRESULT CStrikeJaeger::Initialize_Prototype()
{
...
HRESULT CStrikeJaeger::Initialize(INIT_DESC* pArg)
{
...
void CStrikeJaeger::Update(_float dt)
{
...
void CStrikeJaeger::Late_Update(_float dt)
{
...
void CStrikeJaeger::OnPooledAcquire(INIT_DESC* pArg)
{
	Initialize(pArg);
}

void CStrikeJaeger::OnPooledRelease()
{
}
...
CStrikeJaeger* CStrikeJaeger::Create()
{
	CStrikeJaeger* instance = new CStrikeJaeger();

	if (FAILED(instance->Initialize_Prototype()))
	{
		Safe_Release(instance);
		MSG_BOX("Failed to create : CStrikeJaeger");
	}

	return instance;
}

CGameObject* CStrikeJaeger::Clone(INIT_DESC* pArg)
{
	CStrikeJaeger* instance = new CStrikeJaeger(*this);

	if (FAILED(instance->Initialize(pArg)))
	{
		Safe_Release(instance);
		MSG_BOX("Failed to clone : CStrikeJaeger");
	}

	return instance;
}

void CStrikeJaeger::Free()
{
	__super::Free();
...
void CStrikeJaeger::TakeDamage(DAMAGE_TYPE eDamageType, _float fDamage, CHARACTER charaName)
{
...
/* For.State Machine */
HRESULT CStrikeJaeger::Initialize_StateMachine()
{
	m_pStateMachine = CStateMachine<CStrikeJaeger>::Create();
...
HRESULT CStrikeJaeger::Initialize_States()
{
	m_pStateMachine->Register_State("Born", CStrikeJaeger_Born::Create());
	m_pStateMachine->Register_State("Idle", CStrikeJaeger_Idle::Create());
	m_pStateMachine->Register_State("Attack", CStrikeJaeger_Attack::Create());
	m_pStateMachine->Register_State("Move", CStrikeJaeger_Move::Create());
	m_pStateMachine->Register_State("Chase", CStrikeJaeger_Chase::Create());
	m_pStateMachine->Register_State("Death", CStrikeJaeger_Death::Create());
	m_pStateMachine->Register_State("Groggy", CStrikeJaeger_Groggy::Create());
	m_pStateMachine->Register_State("Hit", CStrikeJaeger_Hit::Create());
	m_pStateMachine->Register_State("Parried", CStrikeJaeger_Parried::Create());

	return S_OK;
}

HRESULT CStrikeJaeger::Initialize_Transitions()
{
	...
	m_pStateMachine->Register_Transition("Idle", "Attack",
		CStateMachine<CStrikeJaeger>::CONDITION_TRIGGER, "Idle_To_Attack");
	m_pStateMachine->Register_Transition("Idle", "Move",
		CStateMachine<CStrikeJaeger>::CONDITION_TRIGGER, "Idle_To_Move");
	m_pStateMachine->Register_Transition("Idle", "Chase",
		CStateMachine<CStrikeJaeger>::CONDITION_TRIGGER, "Idle_To_Chase");
	m_pStateMachine->Register_Transition("Idle", "Death",
		CStateMachine<CStrikeJaeger>::CONDITION_TRIGGER, "Idle_To_Death");
...
void CStrikeJaeger::Update_States(_float dt)
{
	if (true == m_isIdle) {
...
void CStrikeJaeger::ControlState(const _float dt)
{
...
void CStrikeJaeger::CheckDistanceFromPlayer()
{
